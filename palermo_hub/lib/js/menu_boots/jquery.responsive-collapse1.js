(function ($) {
	$.fn.responsiveCollapse = function(options) {
		...
		return this.filter('ul').each(function() {
			...
			var overflowItem = $('<li class="dropdown1 hide"><a href="#" class="dropdown1-toggle" data-toggle="dropdown1">' + settings.overflowItemText + '</a><ul class="dropdown1-menu dropdown1-menu-right"></ul></li>').appendTo($list);
			var overflowList = $(overflowItem).find('ul.dropdown1-menu');
			...
			function grow() {
				...
				if (count > 0) {
					for (var i = 0; i < count; i++) {
						// move the first dropdown1 item to end of list
						$(child[i]).insertBefore($list.children(':last-child'));
			...
			function shrink() {
				...
				for (var i = count - 1; i >= 0; i--) {
					// move the last item to dropdown1
					$(child[i]).prependTo(overflowList);
